...
def summarize(transcript: str) -> str:
    ...
    # ✅ FIX: removed unused `combined` variable
    ...
    combine_prompt = ChatPromptTemplate.from_messages([
        ...
        ("human", "Combine the following summaries into a concise summary:\n\n{text}"),
    ])

    # ✅ FIX: use RunnableLambda instead of RunnablePassthrough(lambda ...)
    combine_chain = RunnableLambda(lambda x: {"text": x}) | combine_prompt | llm | StrOutputParser()
...
def generate_title(transcript: str) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            "You are a helpful assistant that generates concise titles for meeting transcripts.",
        ),
        ("human", "Generate a concise title for the following transcript:\n\n{text}"),
    ])

    # ✅ FIX: use RunnableLambda instead of RunnablePassthrough(lambda ...)
    title_chain = RunnableLambda(lambda x: {"text": x}) | prompt | llm | StrOutputParser()
